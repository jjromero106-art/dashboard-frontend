...
function AirBeamStation5({ onBack, onSelectStation }) {
  ...
  const stationId = 'airbeam3_5';
  const stationName = 'Estación 5';
  const sensorId = 'AirBeam3-409151117128';
  ...
  useEffect(() => {
    const fetchLatestData = async () => {
      ...
      if (!data || data.length === 0) {
        setLatestData({ temperature: 0, humidity: 0, pm1: 0, pm25: 0, pm10: 0, timestamp: new Date().toISOString() });
        return;
      }
  ...
  const fetchChartDataByRange = useCallback(async (range, forceRefresh = false) => {
    const cacheKey = `station5_${range}`;
    const cacheTimeKey = `station5_${range}_time`;
  ...
  useEffect(() => {
    ...
    const cacheKey = `station5_${timeRange}`;
            ...
            <div key={categoryKey} className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {variables.filter(v => v.category === categoryKey).map((v) => {
                  const val = latestData ? latestData[v.key] : 0;
                  const prevVal = previousData ? previousData[v.key] : null;
...
export default AirBeamStation5;