...
function AirBeamStation2({ onBack, onSelectStation }) {
  ...
  const stationId = 'airbeam3_2';
  const stationName = 'Estación 2';
  const sensorId = 'AirBeam3-943cc67e9598';
  ...
  useEffect(() => {
    const fetchLatestData = async () => {
      ...
      // INTERCAMBIO DE DATOS ESTACIÓN 2:
      // PM1 → Temperatura, PM2.5 → Humedad, Temperatura → PM1, Humedad → PM2.5
      const formatted = [
        latest.pm1 || 0,        // PM1 se muestra como Temperatura
        latest.pm25 || 0,       // PM2.5 se muestra como Humedad
        latest.temperature || 0, // Temperatura se muestra como PM1
        latest.humidity || 0,    // Humedad se muestra como PM2.5
        latest.pm10 || 0,
        latest.timestamp
      ];
      
      const prevFormatted = previous ? [
        previous.pm1 || 0,        // PM1 se muestra como Temperatura
        previous.pm25 || 0,       // PM2.5 se muestra como Humedad
        previous.temperature || 0, // Temperatura se muestra como PM1
        previous.humidity || 0,    // Humedad se muestra como PM2.5
        previous.pm10 || 0,
      ] : null;
  ...
  const fetchChartDataByRange = useCallback(async (range, forceRefresh = false) => {
    const cacheKey = `station2_${range}`;
    const cacheTimeKey = `station2_${range}_time`;
    ...
    // Configuración de intervalos como CalidAir
    const intervalMap = {
      '24h': { limit: 288 },   // Últimas 288 lecturas
      '7d': { limit: 336 },    // Últimas 336 lecturas
      '1m': { limit: 360 },    // Últimas 360 lecturas
    };
    
    const { limit } = intervalMap[range] || intervalMap['24h'];
    
    // Consulta directa como CalidAir pero sin función SQL
    ...
    if (data && !error && data.length > 0) {
      // INTERCAMBIO DE DATOS ESTACIÓN 2 PARA GRÁFICOS:
      const formatted = data.reverse().map(d => ({
        timestamp: d.timestamp,
        temperature: parseFloat(d.pm1) || 0,        // PM1 se muestra como Temperatura
        humidity: parseFloat(d.pm25) || 0,          // PM2.5 se muestra como Humedad
        pm1: parseFloat(d.temperature) || 0,        // Temperatura se muestra como PM1
        pm25: parseFloat(d.humidity) || 0,          // Humedad se muestra como PM2.5
  ...
  useEffect(() => {
    ...
    const cacheKey = `station2_${timeRange}`;
                    ...
                    {chartData.length === 0 ? (
                      <div className="flex items-center justify-center h-96 text-gray-500">
                        <div className="text-center">
                          ...
                          <p className="text-sm">No hay datos para el rango de {timeRange === '24h' ? '24 horas' : timeRange === '7d' ? '7 días' : '30 días'} seleccionado</p>
                        </div>
                      </div>
                    ) : (
...
export default AirBeamStation2;