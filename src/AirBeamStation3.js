...
function AirBeamStation3({ onBack, onSelectStation }) {
  ...
  const stationId = 'airbeam3_3';
  const stationName = 'Estación 3';
  const sensorId = 'AirBeam3-1c9dc2f1a0f0';
  ...
  useEffect(() => {
    const fetchLatestData = async () => {
      ...
      const formatted = [latest.pm10 || 0, latest.pm25 || 0, latest.pm1 || 0, latest.humidity || 0, latest.temperature || 0, latest.timestamp];
      const prevFormatted = previous ? [previous.pm10 || 0, previous.pm25 || 0, previous.pm1 || 0, previous.humidity || 0, previous.temperature || 0] : null;
  ...
  const fetchChartDataByRange = useCallback(async (range, forceRefresh = false) => {
    const cacheKey = `station3_${range}`;
    const cacheTimeKey = `station3_${range}_time`;
    ...
    if (data && !error && data.length > 0) {
      const formatted = data.reverse().map(d => ({ timestamp: d.timestamp, temperature: parseFloat(d.pm10) || 0, humidity: parseFloat(d.pm25) || 0, pm1: parseFloat(d.pm1) || 0, pm25: parseFloat(d.humidity) || 0, pm10: parseFloat(d.temperature) || 0 }));
  ...
  useEffect(() => {
    ...
    const cacheKey = `station3_${timeRange}`;
...
export default AirBeamStation3;